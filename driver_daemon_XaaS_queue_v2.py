...
def trigger_process():
    ...
    if len(child_process) < 5:
        ...
        try:
            ...
            processed_list.append(commit_id)
...
#github_name_list = ["git_repo_1" , "git_repo_2" , "git_repo_3"]

#with open('file.txt', 'r') as infile:
#    data = infile.read()
#git_file_list = data.splitlines()
...
while True:
    ...
    list1 = []
    for repo in range(0 , len(processed_list) , 2):
        if processed_list[repo] not in git_file_list:
            list1.append(li1[repo])
    for repo in list1:
        ind = li1.index(repo)
        del li1[ind]
        del li1[ind]    
    count = 0
    for git in git_file_list:
        if git not in processed_list:
             processed_list.append(git)

    
        #index = git_file_list.index(git)
        #out1 = str(eval(git))
        cmd = "git ls-remote https://github.com/" + git + " HEAD | tail -1"
        out = commands.getoutput(cmd)
        out = out.split()[0]
        if(out in processed_list):
            logging.info("commit_id from repo_1 is already processed going for 60 sec sleep : " + str(out)) 
            #time.sleep(60)

        else:
            #git_name = github_name_list[index]
            #commit_list.append(git_name)
            commit_list.append(git)
            logging.info("commit_list : " + str(commit_list))
            commit_list.append(out)
            logging.info("commit_list : " + str(commit_list))
            if(len(child_process) < 5):
                trigger_process()
